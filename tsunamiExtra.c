...
int main(int argc, char **argv){
	...
	if(argc != 3 || atoi(argv[1]) <= 0 || atoi(argv[2]) <= 0 ){
		printf("Error en los parametros de ejecucion. Ejecute como ./ejecutable maxSolicitudes[>0] atendedoresPro[>0]\n");
	} else {
		tamCola=atoi(argv[1]);
		numAtendedoresPRO = atoi(argv[2]);
		printf("Cola:%d \n", tamCola);
		colaSolicitudes = (Solicitud*)malloc(sizeof(Solicitud)*tamCola);
		idAtendedores = (int*)malloc(sizeof(int)*numAtendedoresPRO);
		hiloAtendedores = (pthread_t*)malloc(sizeof(pthread_t)*numAtendedoresPRO);
